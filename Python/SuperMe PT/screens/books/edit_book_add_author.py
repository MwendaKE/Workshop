...
class EditBookAddAuthorWindow(Screen):
    ...
    def __init__(self, **kwargs):
        super(EditBookAddAuthorWindow, self).__init__(**kwargs)
    ...
    def add_book_author(self):
        if (self.author_name.text == "" or self.author_name.text.isspace() or
            ...
            show_error(Title.add_book_author, Error.required)
            ...
            self.refresh_addbook_window()
            
    def refresh_addbook_window(self):
        ao = Book(DB)
        
        self.manager.get_screen('editbook').ids.book_author_spinner.values = ao.load_book_authors()
        self.manager.get_screen('editbook').ids.book_author_spinner.text = self.authorname
        
        self.manager.current = "editbook"
